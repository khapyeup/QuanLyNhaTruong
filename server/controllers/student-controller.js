...
const getStudentByUser = async (req, res) => {
  try {
    ...
    const students = await Student.find({ user_id: userId }).populate(
      "class_id"
    );
...
const getTotalAttendance = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      percent:
        Math.round((studentsAttendance.length / students.length) * 100) || 0,
...
const setAttendanceByClass = async (req, res) => {
  ...
  try {
    const promises = Object.entries(attendance).map(
      async ([studentId, attendanceStatus]) => {
    ...
    res
      .status(200)
      .json({
        message: "Attendance status updated successfully for all students",
      });
  } catch (error) {
