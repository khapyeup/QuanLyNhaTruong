...
export const getProgressRecordDetail = async (req, res) => {
  try {
    const { id } = req.params;

    const progressRecord = await ProgressRecord.findById(id);
    if (!progressRecord)
...
export const addEvidence = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi thêm dẫn chứng " + error.message });
  }
};
