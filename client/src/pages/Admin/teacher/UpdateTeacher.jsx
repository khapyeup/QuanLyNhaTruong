...
import { useAddTeacherMutation, useEditTeacherMutation, useGetTeacherDetailsQuery } from "../../../redux/teacherRelated/teacherApiSlice";
...
const UpdateTeacher = () => {
  ...
  const {id} = useParams();
  const {data: teacherDetails, isLoading: isLoadingTeacher} = useGetTeacherDetailsQuery(id);
  ...
  if (teacherDetails && !isLoadingTeacher)
  {
    setValue("username", teacherDetails.username)
    setValue("password", teacherDetails.password)
    setValue("name", teacherDetails.teacherInfo.name)
    setValue("gender", teacherDetails.teacherInfo.gender)
    setValue("age", teacherDetails.teacherInfo.age)
    setValue("phone", teacherDetails.teacherInfo.phone)
    setValue("email", teacherDetails.teacherInfo.email)
    setValue("class_id", teacherDetails.teacherInfo.class._id)
    setValue("activityAssign", teacherDetails.teacherInfo.activityAssign._id)
  }
  ...
  useEffect(() => {
    setAvatarUrl(teacherDetails?.profile)
  }, [teacherDetails])
  return (
    <>
      <div className="flex flex-col gap-6 p-10">
        <h1 className="font-bold text-2xl">Chỉnh sửa giáo viên</h1>
        <form
