...
import { useGetPaymentDetailQuery } from "../../redux/paymentRelated/paymentApiSlice"
...
export default function PaymentDetail() {
   ...
   //Total paid
    ...
    data.filter(item => item.status !== "Chưa trả").reduce((prev, curr) => {
        return curr.payment.reduce((pre, cur) => cur.amount + pre, 0) + prev;
    }, 0)
    || 0
    const notPaidAmount = data && 
    data.reduce((pre, curr) => curr.balance + pre, 0) 
    || 0;
    
    
    return <div className=" p-10 flex flex-col gap-6">
        ...
        {isLoading ? (
            ...
            <div className="p-4 rounded-lg shadow-md border">
                ...
                <div className="flex flex-col gap-4">
                    {data.map((item) => (
                        <div key={item._id}>
                            <div
                                className={`flex justify-between ${item.status === "Chưa trả"
                                    ? `bg-red-400`
                                            ...
                                            <tbody>
                                                {item.payment.map((payment, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td className="p-4 border-b border-gray-300">{payment.amount.toLocaleString()} VNĐ</td>
                                                        <td className="p-4 border-b border-gray-300">{payment.discount.toLocaleString()} VNĐ</td>
                                                        <td className="p-4 border-b border-gray-300 flex gap-1 items-center flex-row">{payment.paymentMethod === "Tiền mặt" ? <FaMoneyBillAlt className="size-8" /> : <FaRegCreditCard className="size-8" />}  {payment.paymentMethod}
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="shadow-md p-4 border  rounded-lg flex gap-4 flex-col">
                    ...
                    <span className="text-green-500 text-xl font-bold">{paidAmount.toLocaleString()} VNĐ</span>
                </div>
                <div className="shadow-md p-4 border  rounded-lg flex gap-4 flex-col">
                    <h2 className="text-2xl">Tổng tiền chưa thanh toán</h2>
                    <span className="text-red-500 text-xl font-bold">{notPaidAmount.toLocaleString()} VNĐ</span>
                </div>
            </div>
        </div>}
    </div>
